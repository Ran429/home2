...
import { z } from "zod";
import createBoardAction from "./create.action";
...
export default function CreateBoardForm({ adminName }: Props) {
  ...
  async function handleSave(formData: FormData) {
    ...
    if (!response.ok) {
      setError({ paramError: response.paramError, error: response.error });
  ...
  // maintenance/create-form.tsx의 handleFile 함수 방식을 참고하여 구현
  ...
  return (
    <form className="w-full flex flex-col gap-10 mt-10" action={handleSave}>
      ...
      <AdminFormButtonGroup backHref="/admin/board" />
    </form>
  );
}
